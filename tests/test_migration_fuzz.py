...
class TestRestoreContract(unittest.TestCase):
    ...
    def test_credential_isolation_contract(self):
        ...
        self.assertNotIn("exportSecret", self.qml)
        self.assertNotIn("restoreSecret", self.qml)
        self.assertNotIn("pendingSecret", self.qml)
        self.assertIn("stdinEnabled: true", self.qml)
        self.assertIn("clearEnvironment: true", self.qml)
